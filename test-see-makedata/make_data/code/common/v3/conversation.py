...
class conversation_v3:
    def __init__(self, agent_list, customer_list, audio_list, content_list, work_session_number):
        '''
        加载会话必要参数
        ...
        # 坐席参数
        agent_id, agent_name, agent_group, business_type = agent_list[random.randint(0,1)]
        ...
        # 工单会话数量
        self.work_session_number = work_session_number
    ...
    def conversation_work(self):
        conversation = dict()
        conversation["workOrderNo"] = str(uuid.uuid4())[0:17]
        conversation["sessionCount"] = self.work_session_number
        conversation["agentId"] = self.agent_id
        conversation["agentName"] = self.agent_name
        conversation["agentGroup"] = self.agent_group
        conversation["customName"] = self.customer_name
        conversation["workOrderTime"] = datetime.strftime(datetime.now() - timedelta(minutes=random.randint(20, 25)), "%Y-%m-%d %H:%M:%S")
        conversation["dynamicLabel"] = {}
        conversation["recordList"] = list()

        for index in range(1, self.work_session_number + 1):
            audio_add, audio_duration = self.audio_list[random.randint(0, 9)]
            start_time = datetime.now() - timedelta(minutes=random.randint(83, 87))
            end_time = datetime.now() - timedelta(minutes=random.randint(78, 82))
            record = {
                "recordId": str(uuid.uuid4())[0:17],
                "recordAddress": audio_add,
            ...
            conversation["recordList"].append(record)
